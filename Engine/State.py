...
class MR_State(Agent):
    def __init__(self, update_rate, start_time, pos, vel, pos_err, vel_err, wind_spd, aircraft_type, rpy=(0, 0, 0)):
        ...
        self.simstate = 1
        self.trajectory = []
        self.max_thrust = 3 * self.aircraft_type.mass * 9.81  # place a limit on the maximum thrust achievable
    ...
    def update(self, time, drag_model, controller,
               pos_nav_agent, vel_nav_agent, accel_nav_agent,
               air_speed_sensor, windfield):
        if controller.mission_mode == 'Terminated':
            # my_print('Final WPT reached')
            self.simstate = 0
            ...
            achieved_thrust = np.dot(np.array([0, 0, thrust[2]]), cur_rotation.T)
            if np.arcsin(np.sqrt(norm[0] ** 2 + norm[1] ** 2)) < np.pi / 2:
                renorm = target_force[2] / achieved_thrust[2]
                achieved_thrust = renorm * achieved_thrust
            # if np.linalg.norm(achieved_thrust) > self.max_thrust:
            #     my_print('I want too much thrust!!! %.2f'%time)
            #     renorm = self.max_thrust / np.linalg.norm(achieved_thrust)
            #     achieved_thrust = renorm * thrust

            # if np.arcsin(np.sqrt(norm[0] ** 2 + norm[1] ** 2)) < np.pi / 2 - 0.1:
            #     renorm = target_force[2] / thrust[2]
            #     thrust = renorm * thrust
            # if np.linalg.norm(thrust) > self.max_thrust:
            #     my_print('I want too much thrust!!! %.2f'%time)
            #     renorm = self.max_thrust / np.linalg.norm(thrust)
            #     thrust = renorm * thrust
            # achieved_thrust = np.dot(np.array([0, 0, thrust[2]]), cur_rotation.T)
